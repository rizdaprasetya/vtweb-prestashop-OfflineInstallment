...
class VeritransInstallment extends PaymentModule
{
	private $_html = '';
	...
	public function __construct()
	{
		...
		$this->config_keys = array(			
			...
			'VI_KURS',
			'VI_CONVENIENCE_FEE',
			'VI_PAYMENT_SUCCESS_STATUS_MAP',
			'VI_PAYMENT_FAILURE_STATUS_MAP',
			'VI_PAYMENT_CHALLENGE_STATUS_MAP',
			'VI_ENVIRONMENT',
			'VI_SANITIZED',
	...
	public function install()
	{
		...
		$order_state->module_name = 'veritransinstallment';
		...
		Configuration::updateValue('VI_API_VERSION', 2);
		...
		include_once(_PS_MODULE_DIR_ . '/' . $this->name . '/vtinstallment_install.php');
		$vtinstallment_install = new VeritransInstallmentInstall();
		$vtinstallment_install->createTable();
	...
	private function _postValidation()
	{
		...
		{
			if (Tools::getValue('VI_API_VERSION') == 2)
			{
				if (!Tools::getValue('VI_SERVER_KEY'))
					$this->_postErrors[] = $this->l('Server Key is required.');
			} 

			// validate conversion rate existence
			if (!Currency::exists('IDR', null) && !Tools::getValue('VI_KURS'))
			{
	...
	private function _displayVeritransInstallment()
	{
	...
	private function _displayVeritransInstallmentOld()
	{
		$this->_html .= '<img src="../modules/veritransinstallment/Veritrans.png" style="float:left; margin-right:15px;"><b>'.$this->l('This module  offline installment payment via veritrans.').'</b><br/><br/>
			...
			'form' => array(
				...
				/*	array(
					...
					array(
						...
						'name' => 'VI_PAYMENT_SUCCESS_STATUS_MAP',
						'required' => true,	
						'options' => array(
							'query' => $order_states,
							'id' => 'id_option',
					...
					array(
						...
						'name' => 'VI_PAYMENT_FAILURE_STATUS_MAP',
						'required' => true,
						'options' => array(
							'query' => $order_states,
							'id' => 'id_option',
					...
					array(
						...
						'name' => 'VI_PAYMENT_CHALLENGE_STATUS_MAP',
						'required' => true,
						'options' => array(
							'query' => $order_states,
							'id' => 'id_option',
	...
	private function _displayFormOld()
	{
		...
		$this->context->smarty->assign(array(
			...
			'api_version' => htmlentities(Configuration::get('VI_API_VERSION'), ENT_COMPAT, 'UTF-8'),
			'api_versions' => array(1 => 'v1', 2 => 'v2'),
			//'payment_type' => htmlentities(Configuration::get('VI_PAYMENT_TYPE'), ENT_COMPAT, 'UTF-8'),
			//'payment_types' => array('vtweb' => 'VT-Web', 'vtdirect' => 'VT-Direct'),
			// 'client_key' => htmlentities(Configuration::get('VI_CLIENT_KEY'), ENT_COMPAT, 'UTF-8'),
			'server_key' => htmlentities(Configuration::get('VI_SERVER_KEY'), ENT_COMPAT, 'UTF-8'),
			'environments' => array(false => 'Development', true => 'Production'),
			'environment' => htmlentities(Configuration::get('VI_ENVIRONMENT'), ENT_COMPAT, 'UTF-8'),
			// 'enable_3d_secure' => htmlentities(Configuration::get('VI_3D_SECURE'), ENT_COMPAT, 'UTF-8'),
			// 'enable_sanitized' => htmlentities(Configuration::get('VI_SANITIZED'), ENT_COMPAT, 'UTF-8'),
			...
			'payment_success_status_map' => htmlentities(Configuration::get('VI_PAYMENT_SUCCESS_STATUS_MAP'), ENT_COMPAT, 'UTF-8'),
			'payment_challenge_status_map' => htmlentities(Configuration::get('VI_PAYMENT_CHALLENGE_STATUS_MAP'), ENT_COMPAT, 'UTF-8'),
			'payment_failure_status_map' => htmlentities(Configuration::get('VI_PAYMENT_FAILURE_STATUS_MAP'), ENT_COMPAT, 'UTF-8'),
			'kurs' => htmlentities(Configuration::get('VI_KURS', $this->veritrans_kurs), ENT_COMPAT, 'UTF-8'),
			//'kurs' => htmlentities(Configuration::get('VI_INSTALLMENTS_BNI', ENT_COMPAT, 'UTF-8'),
			'convenience_fee' => htmlentities(Configuration::get('VI_CONVENIENCE_FEE', $this->veritrans_convenience_fee), ENT_COMPAT, 'UTF-8'),
	...
	public function getContent()
	{
		...
		$this->_displayVeritransInstallment();
		$this->_displayForm();
	...
	public function hookDisplayPayment($params)
	{
		...
		if($num_product == 1 && Configuration::get('VI_ENABLE_INSTALLMENT') == 'certain_product'){
		...
		if(strlen($displayname) <= 1){
			$displayname = "Credit Card Installment";
		}
	...
	public function execPayment($cart)
	{
		...
		$smarty->assign(array(
			'payment_type' => Configuration::get('VI_PAYMENT_TYPE'),
			'api_version' => Configuration::get('VI_API_VERSION'),
			'error_message' => '',
			'link' => $link,
...
//
	public function getTermInstallment($name_bank){
		$ans = array();
		foreach ($this->config_keys as $key) {
			if ( (strpos($key, 'VI_INSTALLMENTS_' . $name_bank ) !== FALSE) && (Configuration::get($key) == 'on') ){
				
				$term = Configuration::get('VI_INSTALLMENTS_'.$name_bank);
	...
	public function execValidation($cart)
	{
		...
		foreach (Module::getPaymentModules() as $module)
			if ($module['name'] == 'veritransinstallment')
			{
				$authorized = true;
				break;
			}
		...
		//SETUP
		Veritrans_Config::$serverKey = Configuration::get('VI_SERVER_KEY');
		Veritrans_Config::$isProduction = Configuration::get('VI_ENVIRONMENT') == 'production' ? true : false;
	...
	// if (Configuration::get('VI_SANITIZED') == 'on' || Configuration::get('VI_SANITIZED') == 1)
			Veritrans_Config::$isSanitized = false;
		
		//error_log('sanitized '.Configuration::get('VI_SANITIZED'));
		...
		if ($cart_currency->iso_code != 'IDR')
		{
			...
			{
			  // use rate
			  $conversion_func = function($input) { return $input * intval(Configuration::get('VI_KURS')); };
			}
		...
		$installment_type_val = Configuration::get('VI_ENABLE_INSTALLMENT');
		$param_required;
		$offline_term = array();
		switch ($installment_type_val) {
			case 'all_product':
				...
				if ($isBniInstallment){					
					...
					$a = Configuration::get('VI_INSTALLMENTS_BNI');
					$term = explode(',',$a);
					$bni_term = $term;
				...
				if ($isOfflineInstallment){
					...
					$a = Configuration::get('VI_INSTALLMENTS_TERM');
					$term = explode(',',$a);
					$offline_term = $term;
			...
			case 'certain_product':
				...
				if($num_product == 1){
					...
					foreach($attr_product as $att){
						...
						if(strtolower($att_arr[0]) == 'installment' && strtolower($att_arr[1]) == 'cc'){
							$fullPayment = false;
							$param_installment = new stdClass();
		...
		$a = Configuration::get('VI_BIN_FILTER');
	...
	//error_log($param_installment,true);
		$param_payment_option = array(
			'installment' => array(
								...
								"offline_installment_terms" => $offline_term,
		...
		if( !$warning_redirect && 
			($isBniInstallment || $isOfflineInstallment) && 
			(!$fullPayment)  ){

			$params_all['vtweb']['payment_options'] = $param_payment_option;		
		}

		if (Configuration::get('VI_API_VERSION') == 2 && Configuration::get('VI_PAYMENT_TYPE') != 'vtdirect') //transaksi https://github.com/veritrans/veritrans-php/blob/vtweb-2/examples/v2/vt_web/checkout_process.php line 77
		{						
			try {
	...
	public function execNotification()
	{
		...
		//{
			...
			if ($veritrans_notification->transaction_status == 'capture')				
		    {
		     	...
		     	{
		       		$history->changeIdOrderState(Configuration::get('VI_PAYMENT_SUCCESS_STATUS_MAP'), $order_id_notif);
		       		echo 'Valid success notification accepted.';
		       	}
		       	else if ($veritrans_notification->fraud_status== 'challenge')
		     	{
		       		$history->changeIdOrderState(Configuration::get('VI_PAYMENT_CHALLENGE_STATUS_MAP'), $order_id_notif);
		     	...
		     	$history->changeIdOrderState(Configuration::get('VI_PAYMENT_FAILURE_STATUS_MAP'), $order_id_notif);
		       	echo 'Pending notification accepted.';
